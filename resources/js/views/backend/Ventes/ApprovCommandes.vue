        ...
        <v-flex md12>
            ...
            <v-flex xs12 sm12 md6 lg6>
                ...
                <v-autocomplete label="Selectionnez la Commande" prepend-inner-icon="home"
                    ...
                    chips clearable  @change="fetchListDataCommande(svData.refRecquisition)">
                    </v-autocomplete>
                    </div>
                </v-flex>
            ...
            <v-simple-table>
                ...
                <tbody>
                    <tr v-for="(item, index) in svData.detailData" :key="index">
                        <td class="short-cell">
                            <v-text-field v-model="item.idStockService" label="N°Prod" readonly></v-text-field>
                        </td>
                        <td class="long-cell">
                        ...
                        <td class="short-cell">
                            <v-text-field v-model="item.qteCmd" label="Qté Cmd" readonly></v-text-field>
                        </td>
                        <td class="short-cell">
                            <v-text-field v-model="item.qteTempo" label="Qté Restante" readonly></v-text-field>
                        </td>
                        <td class="short-cell">
                            <v-text-field v-model="item.qteEntree" type="number" label="Qté Appro" :rules="[rules.required]"
            ...
            <table>
                <tr>
                    ...
                    <td>
                        <div style="text-align: right; margin-top: 20px;"> <v-btn @click="validate" color="success"  :loading="loading">Enregistrer</v-btn></div>  
...
export default {
            ...
            try
            {
                ...
                if (this.$refs.form.validate()) {
                    ...
                    this.svData.author = this.userData.name;
                        ...
                        this.insertOrUpdate(
                        `${this.apiBaseURL}/insert_vente_global_entree_quick_commande`,
    ...
    showFournisseur() {
      ...
      if (refEnteteEntree != '') {
        ...
        async fetchListDataCommande(code) {
            try {
                const response = await this.editOrFetch(`${this.apiBaseURL}/fetch_data_commande/${code}`);
                const { data } = response;
                ...
                if (data && data.data) {
                    ...
                    this.svData.detailData = donnees.map((item, index) => {
                        ...
                        return {
                            ...
                            qteDisponible: 0,
                            qteEntree: item.resteCmd,
                            qteCmd: item.qteCmd,
                            puEntree: item.puCmd,                    
                            montantreduction: item.montantreduction,
                            pt: item.PTCmd,
                            tva:item.montanttva,
                            montant_tva: item.montanttva,
                            idStockService : item.idStockService,
                            nom_unite : item.uniteCmd,
                            refProduit : item.refProduit,
                            designation : item.designation,
                            qteTempo : item.resteCmd,
                        };
                    });
                } else {
