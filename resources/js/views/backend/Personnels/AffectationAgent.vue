<template>
  <v-row justify="center">
    ...
    <PersenceAgent ref="PersenceAgent" />
    <Mission ref="Mission" />
    <!-- Mission -->
                    ...
                    <v-form ref="form" lazy-validation>
                      ...
                      <v-card-text>
                        <v-layout row wrap>
                          ...
                          <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                              <v-autocomplete label="Selectionnez la Categorie" prepend-inner-icon="mdi-map"
                                :rules="[(v) => !!v || 'Ce champ est requis']" :items="categorieList" item-text="name_categorie_agent"
                          ...
                          <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                              <v-autocomplete label="Selectionnez le lieu d'Affectation" prepend-inner-icon="mdi-map"
                                ...
                                item-value="id" dense outlined v-model="svData.refLieuAffectation" chips clearable>
                              </v-autocomplete>
                            </div>
                          </v-flex>
                          <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                              <v-autocomplete label="Selectionnez le Mutuelle de Santé" prepend-inner-icon="mdi-map"
                                ...
                                item-text="nom_mutuelle" item-value="id" dense outlined v-model="svData.refMutuelle"
                                chips clearable>
                              </v-autocomplete>
                            </div>
                          </v-flex>
                          ...
                          <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                              <v-autocomplete label="Selectionnez le Projet (Administration)" prepend-inner-icon="mdi-map"
                                ...
                                item-text="description_projet" item-value="id" dense outlined
                          ...
                          <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                              <v-autocomplete label="Premier jour de Travail dans la semaine" :items="[
                                ...
                                dense item-text="designation" item-value="designation"
                                v-model="svData.JourTrail1"></v-autocomplete>
                            </div>
                          </v-flex>
                          <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                              <v-autocomplete label="Dernier jour de Travail dans la semaine" :items="[
                                ...
                                dense item-text="designation" item-value="designation"
                                v-model="svData.JourTrail2"></v-autocomplete>
                            </div>
                          </v-flex>
                          ...
                          <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                              <v-text-field type="number" label="Nombre de jour de Congé Annuel"
                                prepend-inner-icon="event" dense :rules="[(v) => !!v || 'Ce champ est requis']" outlined
                                v-model="svData.nbrConge">
                              </v-text-field>
                            </div>
                          </v-flex>
                          <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                              <v-text-field label="Nombre de jour de Congé Annuel en Lettre" prepend-inner-icon="event"
                                dense :rules="[(v) => !!v || 'Ce champ est requis']" outlined
                                v-model="svData.nbrCongeLettre">
                              </v-text-field>
                            </div>
                          </v-flex>
                                ...
                                <td>                                 
                                  ...
                                  <v-menu bottom rounded offset-y transition="scale-transition">
                                    ...
                                    <v-list dense width="">
                                      ...
                                      <v-list-item link @click="showMission(item.id, item.noms_agent)">
                                        <v-list-item-icon>
                                          <v-icon color="  blue">description</v-icon>
                                        </v-list-item-icon>
                                        <v-list-item-title style="margin-left: -20px">Envoie en Mission
                                        </v-list-item-title>
                                      </v-list-item>
                                      ...
                                      <v-list-item link @click="showPersenceAgent(item.id, item.noms_agent)">
                                        <v-list-item-icon>
                                          <v-icon color="  blue">description</v-icon>
                                        </v-list-item-icon>
                                        <v-list-item-title style="margin-left: -20px">Presence pour l'Agent
                                        </v-list-item-title>
                                      </v-list-item>
                                      ...
                                      <v-list-item link @click="printBill(item.id)">
                                        ...
                                        <v-list-item-title style="margin-left: -20px">Contrat de Travail
                                        </v-list-item-title>
                                      </v-list-item>

                                      <v-list-item link @click="printPrestation(item.id)">
                                        <v-list-item-icon>
                                          <v-icon>print</v-icon>
                                        </v-list-item-icon>
                                        <v-list-item-title style="margin-left: -20px">Contrat de Prestation
                                        </v-list-item-title>
                                      </v-list-item>

                                      <v-list-item link @click="printNotification(item.id)">
                                        <v-list-item-icon>
                                          <v-icon>print</v-icon>
                                        </v-list-item-icon>
...
import PersenceAgent from './PersenceAgent.vue';
...
export default {
  ...
  methods: {
    ...
    printBill(id) {
      window.open(`${this.apiBaseURL}/pdf_contrat_travail_agent?id=` + id);
    },
    printPrestation(id) {
      window.open(`${this.apiBaseURL}/pdf_contrat_prestation_agent?id=` + id);
    },
    ...
    showMission(refAffectation, name) {

      if (refAffectation != '') {

        this.$refs.Mission.$data.etatModal = true;
        this.$refs.Mission.$data.refAffectation = refAffectation;
        this.$refs.Mission.$data.svData.refAffectation = refAffectation;
        this.$refs.Mission.fetchDataList();
        this.fetchDataList();

        this.$refs.Mission.$data.titleComponent =
          "Envoie en Mission pour " + name;
    ...
    showPersenceAgent(affectation_id, name) {
      //PersenceAgent
      if (affectation_id != '') {

        this.$refs.PersenceAgent.$data.etatModal = true;
        this.$refs.PersenceAgent.$data.affectation_id = affectation_id;
        this.$refs.PersenceAgent.$data.svData.affectation_id = affectation_id;
        this.$refs.PersenceAgent.fetchDataList();
        this.fetchDataList();

        this.$refs.PersenceAgent.$data.titleComponent =
          "Presence pour " + name;
    ...
    async get_projet_for_Categorie(projet_id) {
      this.isLoading(true);
      await axios
        .get(`${this.apiBaseURL}/fetch_data_salairebase/${projet_id}`)
        .then((res) => {
          var chart = res.data.data;
          if (chart) {
            this.postessList = chart;
          } 
          else {
            this.postessList = [];
          }
