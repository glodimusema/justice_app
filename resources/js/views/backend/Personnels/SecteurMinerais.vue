<template>
              ...
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  <v-text-field label="Designation" prepend-inner-icon="extension" dense
                    :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_secteur">
                  </v-text-field>

                  <v-text-field label="Description" prepend-inner-icon="extension" dense
                    :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.description_secteur">
                  </v-text-field>
                </v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
                ...
                <template v-slot:default>
                  ...
                  <tbody>
                    <tr v-for="item in fetchData" :key="item.id">
                      ...
                      <td>{{ item.description_secteur }}</td>
                      <td>
  ...
  export default {
    ...
    methods: {
      ...
      testTitle() {
        if (this.edit == true) {
          this.titleComponent = "modification de " + item.nom_secteur;
        } else {
          this.titleComponent = "Ajout Categorie";
        }
      }
      ,
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_all_secteur_minerais?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_secteur_minerais`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_secteur_minerais/${id}`).then(
          ({ data }) => {
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_secteur_minerais/${id}`).then(
            ({ data }) => {
