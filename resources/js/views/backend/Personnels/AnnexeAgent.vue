                  ...
                  <v-dialog v-model="dialog" max-width="900px" persistent>
                    <v-card :loading="loading">
                      <v-form ref="form" lazy-validation>
                        <v-card-title>
                          Annexe pour Agent <v-spacer></v-spacer>
    ...
    methods: {
      ...
      updatePhoto() {
        ...
        if (this.edit == true) {
          ...
          axios
            .post(`${this.apiBaseURL}/update_perso_annexe`, formData, config)
            .then(({ data }) => {
              this.image = "";
          ...
          this.svData.author = this.userData.name;
          axios
            .post(`${this.apiBaseURL}/insert_perso_annexe`, formData, config)
            .then(({ data }) => {
              this.image = "";
              this.showMsg(data.data);
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_perso_annexe/${id}`).then(
          ({ data }) => {
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_perso_annexe/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_perso_annexe_agent/${this.refAgent}?page=`);
        //
      },
