                  ...
                  <v-dialog v-model="dialog" max-width="900px" persistent>
                    <v-card :loading="loading">
                      <v-form ref="form" lazy-validation>
                        <v-card-title>
                          Annexe pour Dossier <v-spacer></v-spacer>
                        ...
                        <v-card-text>
                          ...
                          <v-layout row wrap>
                            ...
                            <v-flex xs12 sm12 md12 lg12>
                              <div class="mr-1">
                                <v-text-field label="Designation" prepend-inner-icon="event" dense
                                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.noms_annexe">
                                </v-text-field>
                              </div>
                            </v-flex>  
  ...
  export default {
    ...
    methods: {
      ...
      updatePhoto() {
        ...
        if (this.edit == true) {
          this.svData.id_dossier = this.id_dossier;
          this.svData.author = this.userData.name;
          axios
            .post(`${this.apiBaseURL}/update_jur_annexe_dossier`, formData, config)
            .then(({ data }) => {
              this.image = "";
          ...
          this.svData.author = this.userData.name;
          axios
            .post(`${this.apiBaseURL}/insert_jur_annexe_dossier`, formData, config)
            .then(({ data }) => {
              this.image = "";
              this.showMsg(data.data);
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_jur_annexe_dossier/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
            donnees.map((item) => {
              this.svData.id = item.id;
              this.svData.id_dossier = item.id_dossier;
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_jur_annexe_dossier/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_jur_annexe_bydossier/${this.id_dossier}?page=`);
        //
      }
