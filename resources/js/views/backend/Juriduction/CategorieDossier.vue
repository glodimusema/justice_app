                    ...
                    <v-flex xs12 sm12 md12 lg12>
                      <div class="mr-1">
                        <v-text-field label="Designation" prepend-inner-icon="extension" dense
                          ...
                          v-model="svData.nom_categorie_dossier"></v-text-field>
                      </div>
                    </v-flex>

                    <v-flex xs12 sm12 md12 lg12>
                      <div class="mr-1">
                        <v-text-field label="Code Catégorie" prepend-inner-icon="extension" dense
                          :rules="[(v) => !!v || 'Ce champ est requis']" outlined
                          v-model="svData.code_categorie_dossier"></v-text-field>
                      </div>
                    </v-flex>

                    <v-flex xs12 sm12 md12 lg12>
                      <div class="mr-1">
                        <v-text-field label="Durée" prepend-inner-icon="extension" dense
                          :rules="[(v) => !!v || 'Ce champ est requis']" outlined
                          v-model="svData.duree"></v-text-field>
                      </div>
                    </v-flex>
                ...
                <template v-slot:default>
                  ...
                  <tbody>
                    <tr v-for="item in fetchData" :key="item.id">
                      ...
                      <td>{{ item.code_categorie_dossier }}</td>
                      <td>{{ item.duree }}</td>
                      <td>
  ...
  export default {
    ...
    data() {
      return {
        ...
        disabled: false,
        edit: false,
        //'id','nom_categorie_dossier','code_categorie_dossier','duree'
        svData: {
          id: "",
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout Catégorie Dossier";
        this.edit = false;
      ...
      //   }, 300),
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_jur_categorie_dossier?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_jur_categorie_dossier`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_jur_categorie_dossier/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
  
            donnees.map((item) => {
              this.titleComponent = "modification de " + item.nom_categorie_dossier;
            });
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_jur_categorie_dossier/${id}`).then(
            ({ data }) => {
