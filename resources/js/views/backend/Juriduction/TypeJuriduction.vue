<template>
                    ...
                    <v-flex xs12 sm12 md12 lg12>
                      <div class="mr-1">
                        <v-text-field label="Designation" prepend-inner-icon="extension" dense
                          ...
                          v-model="svData.nom_type_jur"></v-text-field>
                      </div>
                    </v-flex>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout Type";
        this.edit = false;
      ...
      //   }, 300),
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_jur_type_juridiction?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_jur_type_juridiction`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_jur_type_juridiction/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
  
            donnees.map((item) => {
              this.titleComponent = "modification de " + item.nom_type_jur;
            });
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_jur_type_juridiction/${id}`).then(
            ({ data }) => {
